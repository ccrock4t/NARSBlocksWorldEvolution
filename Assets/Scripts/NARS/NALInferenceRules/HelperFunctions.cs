...
public class HelperFunctions
{
    ...
    public string sentence_to_string(Sentence sentence)
    {
        ...
        EvidentialValue projectedValue = this.nars.inferenceEngine.get_sentence_value_time_decayed(sentence);
        //event
...
*/
        ...
        */
        ...
        if (result_type == typeof(Judgment) || result_type == typeof(Goal))
        {
            ...
            if (higher_order_statement)
            {
                ...
                (f2, c2) = (j2.evidential_value.frequency, j2.evidential_value.confidence);
            }
            else
            {
                EvidentialValue j1_value_decayed = this.nars.inferenceEngine.get_sentence_value_time_decayed(j1);
                EvidentialValue j2_value_decayed = this.nars.inferenceEngine.get_sentence_value_time_decayed(j2);
        ...
        */
        ...
        if (result_type == typeof(Judgment) || result_type == typeof(Goal))
        {
            ...
            if (result_truth == null)
            {
                ...
                {
                    result_truth = j.evidential_value; //NALGrammar.Values.EvidentialValue(j.value.frequency,j.value.confidence)
                }
                else
                {
                    result_truth = truth_value_function(j.evidential_value.frequency, j.evidential_value.confidence);
                }
            }
        }
    ...
    public float get_unit_evidence()
    {
        return 1f / (1f + this.nars.config.k);
    }
}